...
/**
 * Synthetic contribution data, deterministic per login. Lets the renderer be
 * worked on without burning API calls, and powers the Pages playground preview.
 * Shaped like a real year: weekday rhythm, a seasonal wave, a hot streak, and
 * a vacation or two of bare dirt.
 */
export function demoLawn(login = "demo", weeks = 53): Lawn {
  ...
  for (let w = 0; w < weeks; w++) {
    ...
    if (vacationWeeks.has(w)) weekBase = 0;
    if (w >= streakStart && w < streakStart + 4) weekBase *= 1.8;
    for (let d = 0; d < 7; d++) {
      const weekend = d === 0 || d === 6;
      const i = weekBase * (weekend ? 0.35 : 1) * (0.6 + rnd() * 0.8);
      const level: Level = i < 0.12 ? 0 : i < 0.3 ? 1 : i < 0.5 ? 2 : i < 0.78 ? 3 : 4;
